...
async function processPhysicalScan(rawData) {
  ...
  try {
    ...
    if (!isJSON) {
      ...
      const isValidFirstChar = ['A', 'B', 'C', 'D', 'E', 'F'].includes(firstChar);
    ...
    try {
      ...
      // Vérifier si le premier caractère est une lettre d'univers (A-F)
      if (['A', 'B', 'C', 'D', 'E', 'F'].includes(firstChar) && upperData.length > 1) {
        qrData = {
          universe: firstChar,
...
export const renderQR = (root) => {
  ...
  qrHome.innerHTML = `
    <div class="home-visuals-overlay" id="homeVisualsOverlay"></div>
    ${statsHtml}
