...
// Récupérer les univers de l'écran d'accueil
ipcMain.handle('admin:get-home-universes', async () => {
  if (!photoSystemReady || !photoSystem?.db) {
    return { status: 'error', error: 'PhotoSystem non disponible' };
  }
  try {
    const universes = await photoSystem.db.getHomeScreenUniverses();
    return { status: 'success', universes };
  } catch (error) {
    console.error('[IPC] Erreur get-home-universes:', error);
    return { status: 'error', error: error.message };
  }
});

// Mettre à jour l'état d'un univers
ipcMain.handle('admin:update-home-universe', async (event, { universeKey, enabled }) => {
  if (!photoSystemReady || !photoSystem?.db) {
    return { status: 'error', error: 'PhotoSystem non disponible' };
  }
  try {
    await photoSystem.db.updateHomeScreenUniverse(universeKey, enabled);
    console.log('[IPC] Univers mis à jour:', universeKey, enabled);
    return { status: 'success', universeKey, enabled };
  } catch (error) {
    console.error('[IPC] Erreur update-home-universe:', error);
    return { status: 'error', error: error.message };
  }
});
...
ipcMain.handle('admin:get-home-visuals', async () => {
  try {
    ...
    // Récupérer les univers activés depuis la DB
    let enabledUniverses = [];
    if (photoSystemReady && photoSystem?.db) {
      enabledUniverses = await photoSystem.db.getEnabledHomeUniverses();
    }

    // Si aucun univers activé, fallback sur tous les sous-dossiers A-F
    if (enabledUniverses.length === 0) {
      enabledUniverses = [
        { universe_key: 'A' },
        { universe_key: 'B' },
        { universe_key: 'C' },
    ...
    // Parcourir chaque univers activé
    for (const universe of enabledUniverses) {
      const universeDir = path.join(visualsDir, universe.universe_key);
      if (fs.existsSync(universeDir)) {
        const entries = fs.readdirSync(universeDir);
        const images = entries
          .filter(f => imageExtensions.includes(path.extname(f).toLowerCase()))
          .map(f => `./assets/visuals/${universe.universe_key}/${f}`);
        allImages.push(...images);
      }
    }

    return { status: 'success', images: allImages };
  } catch (error) {
...
async function fetchProductsFromAPI() {
  ...
  try {
    ...
    console.log('[Products] ═══════════════════════════════════════════════');
    console.log('[Products] Nombre total de produits:', Object.keys(products).length);
  ...
  try {
    ...
    if (photoSystem?.db) {
      ...
      if (!isUpdate) {
        ...
        // Après PUT avec nouveaux items: utiliser /get-order-items pour récupérer les IDs
        console.log('[Sync] 🔗 Nouveaux items créés lors du PUT, récupération des IDs via /get-order-items...');

        try {
          // Utiliser l'endpoint dédié pour récupérer les order_items
          const baseUrl = process.env.BASE_URL || 'https://ygetxuvqrknbggplzmvy.supabase.co/functions/v1';
          const getItemsUrl = `${baseUrl}/get-order-items?order_id=${supabaseOrderId}`;
          console.log('[Sync]   GET URL:', getItemsUrl);
          ...
          // La réponse a le format: { success: true, order_id, count, data: [...items] }
          if (getResponse.success && getResponse.data && getResponse.data.length > 0) {
            const supabaseItems = getResponse.data;
            console.log('[Sync]   📦 Items récupérés via /get-order-items:', supabaseItems.length);
            supabaseItems.forEach((item, idx) => {
              console.log(`[Sync]     Item ${idx + 1}: id=${item.id}, photo_id=${item.photo_id}, product_id=${item.products?.id || item.product_id}, status=${item.status}`);
            });

            await photoSystem.db.updateOrderItemsSupabaseIds(orderId, supabaseItems);
            console.log('[Sync]   ✅ supabase_item_id sauvegardés après PUT');
          } else {
            console.log('[Sync]   ⚠️ Aucun order_items trouvé dans la réponse:', getResponse);
          }
        } catch (getErr) {
